...
In section \ref{patterns-profiles} the general properties of load profiles and how they are influenced by exogenous factors was discussed. Now, these general properties will be investigated in depth for the Bruny Island feeder.
\subsubsection{Available data}
The following data is available:
\begin{itemize}
	\item Apparent power measured at recloser R1 (figure \ref{fig:bruny_network}) January 1, 2007 and June 25, 2018
...
An intuitive reaction to this might be to use different forecasting models for summer and winter - but of course then a delineation must be decided on for when to switch.
...
For the default configuration, the network was supplied with data from the previous and future 24 hours:
\begin{itemize}
	\item The previous 24 hours of load, $\mL = [ L_1 \: L_2 \: ...  \: L_{24}]^\top$, with $L_{24}$ being the most recent load.
	\item Temperature for the future 24 hours, $\mT = [T_1 \: T_2 \: ... \: T_{24}]^\top$.
	\item Day of the week as an integer from 0 to 6 (local time) over the future 24 hours, $\mD = [D_1 \: D_2 \: ... \: D_{24}]^\top$.
	\item Minutes since midnight (local time) for the future 24 hours, $\mM = [M_1 \: M_2 \: ... \: M_{24}]^\top$.
	\item Boolean 1 or 0 indicating whether it is a holiday at each hour over the next 24 hours, $\mH = [H_1 \: H_2 \: ... \: H_{24}]^\top$.
	\item Holiday type as an integer from 0 to $n$ at each hour over the future 24 hours, where there are $n$ different holidays in the year, $\mG = [G_1 \: G_2 \: ... \: G_{24}]^\top$
	\item Month of the year from 0 to 11 (local time) over the future 24 hours, $\mN = [N_1 \: N_2 \: ... \: N_{24}]^\top$.
\end{itemize}
...
\begin{itemize}
	\item 24 hours of similar profile load $\mL_{sk} = [ L_{sk,1} \: L_{sk,2} \: ... \: L_{sk,24}]^\top$.
	\item 24 hours of similar profile temperature, corresponding to the same points in time as $\mL_{sk}$, $\mT_{sk} = [ T_{sk,1} \: T_{sk,2} \: ...  \: T_{sk,24}]^\top$.
\end{itemize}

This data was concatenated to form the input matrix \\
$\mX = [ \mL \: \mT \: \mD \: \mM \: \mH \: \mG \: \mN \: \mL_{s1} \: \mT_{s1} \: \ldots \: \mL_{sk} \: \mT_{sk}] \in \mathbb{R}^{24 \times (7 + 2k)}$.
		...
		\begin{table}[htbp]
			...
			\begin{center}
				\begin{tabular}{llcccccc}
					...
					& SARIMAX           & 00.00 & 00.0 & 00.00 & 00.00 & 000.0 & 000.0        \\
					& S2S      			&  7.85 & 42.8 &  9.87 & 110.6 & 11.42 & 112.6           \\
					& Transformer       &  7.82 & 43.1 & 10.33 & 115.1 & 13.00 & 140.1            \\
					& U. Transformer    &  8.20 & 46.0 & 11.72 & 131.6 & 14.52 & 155.2          \\ \cline{1-8} 
					
					\multirow{3}{*}{\shortstack[l]{Hourly\\with similar\\profiles}} 
					& S2S      			&  7.68 & 41.6 &  9.77 & 109.6 & 11.05 & 118.7           \\
					& Transformer       &  7.40 & 40.7 & 10.25 & 115.2 & 12.45 & 133.6            \\
					& U. Transformer    &  8.15 & 44.8 & 11.10 & 123.5 & 12.67 & 135.5          \\ \cline{1-8} 
					
					\multirow{3}{*}{\shortstack[l]{Half-\\hourly}}       
					& S2S      			&  7.80 & 42.7 &  &  &  &         \\
					& Transformer       &  0    & 1    &  &  &  &          \\
					& U. Transformer    &  0    & 1    &  &  &  &        \\ \cline{1-8}
					...
					\multirow{3}{*}{\shortstack[l]{Hourly\\long}}       
					& S2S      			&  0    & 1    &  &  &  &         \\
					& Transformer       &  0    & 1    &  &  &  &           \\
					& U. Transformer    &  0    & 1    &  &  &   &       \\ \cline{1-8} 
				\end{tabular}
