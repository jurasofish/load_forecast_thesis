...
Load forecasting methods commonly employed in industry are neither intended to forecast with high accuracy over a time period this short nor at such a low level in the distribution network \citep{CIGRE2016}.
...
Generally, a load forecast uses exogenous information, such as weather and date information, as input in order to produce a forecast.
...
In section \ref{background} it was stated that load forecasting methods commonly employed in industry neither intended to forecast with high accuracy over a 24 hour period nor at such a low level in the distribution network.
This is supported by a survey conducted by CIGRE in 2016 \citep{CIGRE2016} (\hl{todo get access to the full version of this instead of just the article summary}) showing that, of 29 utilities, 79\% use load forecasting for long term planning and 32\% for short term operational planning.

Interestingly, this survey also found that the majority of load forecasting software is developed in-house, and the methodology used is usually revised at least every four years. \hl{this section will be further updated after getting full access to the survey}.
