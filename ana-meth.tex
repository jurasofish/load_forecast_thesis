...
The structure of this model is a natural fit for time series forecasting - neural machine translation maps one sequences of words (vectors) to another, just as a time series forecasting system maps a sequence of historical observations (vectors) to a sequence of forecasts.
...
This model is attractive because it may overcome the tendency for recurrent neural networks to ``forget" data from early in the input sequence.
...
In the case of recurrent neural networks, where the outputs of sub-functions in the network form cycles, the network can be unrolled to form a feedforward network.
...
In stochastic gradient descent, $\vx$ and $\vy$ are a randomly selected pair from the training set and so over many training iterations the entire training set will be approximately equally sampled.
