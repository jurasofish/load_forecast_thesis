...
The recurrent neural network RNN architecture, shown in figure \hl{xx}, was introduced by \cite{Rumelhart1986} in 1986.
RNNs operate on sequences of data by applying an identical function to every element of the sequence.
This function takes as input the output of the function at the previous element, and the current element of the sequence.
RNNs suffer from issues with vanishing and exploding gradients \cite{Goodfellow-et-al-2016}, which is addressed through the use of a long short term memory (LSTM) \cite{hochreiter1997long} cell as the function in the RNN.

An LSTM cell is able to selectively add and remove elements from the state as it passes through.
